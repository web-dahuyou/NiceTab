...
- 最新自动快照单独保留一份，用于浏览器启动恢复，不占用手动快照数量。
- 快照支持重命名和删除；其中的标签页支持新增、编辑、删除、排序、跨分组拖动、切换固定状态和设置恢复后激活项。
- 快照中的分组支持新增、重命名、调整颜色和折叠状态、排序及删除。
- 恢复时可选择“在新窗口还原”以保留当前工作，或选择“替换当前窗口”关闭当前窗口原有标签后完整还原。
- 个别网址因浏览器安全限制无法打开时，NiceTab 会继续恢复其他标签页，并显示成功与失败数量。

固定标签页不能属于 Chrome 原生标签组。将分组内标签设为固定时，NiceTab 会自动把它移到快照顶部的未分组区域。不同 Chrome 配置文件或不同设备之间不能直接读取彼此的窗口标签，快照也不会加入 Gist/WebDAV 同步。
