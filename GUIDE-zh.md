...
<p>
  中文 | <a href="./GUIDE.md">English</a> | <a href="./GUIDE-ru.md">Русский</a>
</p>
...
详细的更新记录请查看 <a href="./CHANGELOG-zh.md">变更日志</a>。
...
- 发送所有标签页：将浏览器当前窗口所有打开的标签页发送到 NiceTab 管理页面。
- 发送所有窗口的标签页：将浏览器所有窗口打开的标签页发送到 NiceTab 管理页面。
- 发送当前标签组：将当前激活或高亮的标签页所在的标签组发送到 NiceTab 管理页面（支持多个标签页和跨标签组，浏览器标签页可多选高亮）。
...
**补充说明**：
- v2.8.4 firefox新增标签页右键菜单-发送当前标签页。
...
- 前往：快捷跳转到指定页面
  ...
  - 用户指南：跳转到 NiceTab 用户指南页面。
- 操作：快捷使用 NiceTab 常用功能
  - 发送标签页：包括发送所有标签页、发送当前标签页、发送其他标签页、发送左侧标签页、发送右侧标签页等。
  - 全局搜索
  - 休眠其他标签页：休眠当前标签页之外的标签页。
  - 开始同步：手动快捷执行同步操作。
- 主题：可切换主题色。
...
### 新便签页
v3.0.0 版本新增自定义浏览器新标签页功能和配置项，您可以将新标签页替换为 `NiceTab 管理后台` 或者 `NiceTab 定制新标签页`，默认选项为浏览器原生的新标签页。

可根据自己喜好前往偏好设置页进行设置。

## 管理后台-导航栏操作
- 页面切换：可在**列表**、**快照**、**偏好设置**、**导入/导出**、**远程同步**、**回收站**这些页面快速切换。
- 版本信息展示：日常展示当前版本信息，有可用更新时，会展示最新可升级的版本，点击可立即更新。
- 主题色设置；预设多种主题色，可自由切换（主题色目前只是限定的几种主题色，后续看情况优化暗色主题色及主题色扩展）。
- 亮色/暗色主题：可自由切换亮色/暗色主题。
- 语言切换：可切换语言，目前支持中文、英文、繁体中文、俄文，可自由切换。
- 其他操作项：其他的一些功能折叠在下拉选项中，这里面的功能也都比较实用。

### 窗口快照

点击导航栏“操作”中的“创建快照”，可以一键保存**当前窗口**的完整工作环境。快照包含标签页顺序、固定状态、恢复后激活的标签页，以及原生标签组的名称、颜色和折叠状态。多个 Chrome 窗口会分别保存，不会混合到同一份快照中。

管理后台新增了“快照”页面：

- 手动快照按日期时间自动命名，最多保留 50 份；达到上限时可确认删除最旧快照并继续创建。
- 标签页和分组变化会实时写入崩溃恢复缓冲区；浏览器下次启动时，NiceTab 将该缓冲区固化为一份只读自动快照，并根据“启动浏览器时自动恢复”设置决定是否恢复。
- 快照页使用“手动快照 / 自动快照”侧边导航。主列表只展示名称、更新时间和统计信息，点击“查看”后才在侧滑详情中按需渲染标签页和分组，避免大量标签页拖慢列表。
- 手动快照支持重命名和删除。偏好设置中的“允许编辑手动快照中的标签页和分组”默认关闭；开启后可在详情侧滑框中新增、编辑、删除、排序和跨分组拖动标签页，也可维护固定状态、激活项及分组属性。
- 自动快照只允许查看和恢复，不支持重命名、编辑或删除；下一次浏览器启动时会由最新恢复缓冲区替换。
- 恢复时可选择“在新窗口还原”以保留当前工作，或选择“替换当前窗口”关闭当前窗口原有标签后完整还原。
- 个别网址因浏览器安全限制无法打开时，NiceTab 会继续恢复其他标签页，并显示成功与失败数量。

固定标签页不能属于 Chrome 原生标签组。启用高级编辑后，将分组内标签设为固定时，NiceTab 会自动把它移到快照顶部的未分组区域。不同 Chrome 配置文件或不同设备之间不能直接读取彼此的窗口标签，快照也不会加入 Gist/WebDAV 同步。
...
列表页包括左侧的目录树（分类和标签组目录索引）、中间列表（当前选中的分类中的所有标签组以及标签组中的所有标签页）和右侧已打开标签页面板。
...
- 点击分类和标签组，会选中对应的分类或标签组，同时中间列表会展示该分类下的所有标签组及标签组中的标签页列表。
...
中间列表展示当前选中的分类中的所有标签组以及标签组中的所有标签页，采用虚拟列表渲染。

中间列表以标签组为单位进行展示，这里的标签组支持重命名、删除、打开到浏览器、锁定、星标、拷贝、复制批量链接、去重等操作。

标签组操作：
- 重命名：默认标签组名称 `UNnamed Group`（现在默认名称是创建时间，格式为 `G_20260410_09:01:11_858`），点击标签组名称右侧的编辑图标可进行重命名（有字数限制）。
- 删除：点击标签组的“删除”按钮可删除该标签组，删除前会弹窗确认。
- 打开到浏览器：点击“打开该组”按钮， 则会在浏览器中打开该标签组中的所有标签页（可配置打开标签页后自动从列表中删除）。
- 在上方新建标签组：在当前标签组上方新建标签组。
- 在下方新建标签组：在当前标签组下方新建标签组。
...
- 标签组内的标签页支持多选、拖拽排序
- **支持鼠标框选的方式快速多选标签页**。
- **支持快速批量选中两个标签页之间的所有标签页**：按住shift点击一个标签页，该标签页会高亮，再按住shift点击下一个标签页，批量选中。
- 多选标签页后，支持批量删除、打开、移动到、批量拖拽移动、复制链接、克隆等操作。
- **温馨提示**：您还可以拖拽标签页到左侧目录树的标签组节点实现快捷移动。

### 已打开标签页面板

v2.9.0 版本在页面右侧新增已打开标签页面板

页面右侧的折叠面板展示当前浏览器窗口中打开的标签页和标签组，该面板的标签页支持快捷多选、批量拖拽到页面的中间列表以及左侧的目录树节点中。
...
| 配置项 | 说明 | 默认值 | 关键指数 |
| --- | --- | --- | --- |
| 语言 | 设置插件展示语言，中文、英文、繁体中文、俄文切换 | 根据系统语言自动设置，兜底为英文 | ★★ |
| 主题模式 | 设置插件的亮色/暗色模式，以及跟随系统 | 浅色 | ★★ |
...
| 发送标签页时-以下域名的标签页不会发送到管理后台 | 某些域名的标签页，如：空白页、新标签页等重要性低的标签页不想发送到管理后台，可填写对应的域名，按照提示信息配置 <br /> 正则示例：`.*?\.vuejs\.org.*` <br />通配符示例：`*.vuejs.org*` | 空白页和新标签页的地址 | ★★ |
| 发送标签页时-是否打开NiceTab管理后台 | 发送标签页成功后，是否需要打开NiceTab管理后台，本人喜欢打开管理后台确认一遍，或者进行下一步操作 | 是 | ★★★ |
| 发送单个标签页时是否创建新的标签组 | 若选择“否”，则自动发送到中转站中第一个非星标/非锁定的标签组中 | 是 | ★★★ |
...
| 打开多个标签页时是否自动休眠 | 当从列表中打开标签页/标签组到浏览器时，是否自动休眠 | 否 | ★★★★ |
...
| 批量打开标签页的顺序 | 默认情况下，从左到右依次打开标签页，有的用户浏览器设置了在当前页右侧打开标签页，则配置项可以设置为“反向”，让标签页按照正常顺序打开。 | 默认 | ★★ |
...
自定义改写网页的标题，以前专门安装的修改网页标题的插件可以卸载掉了。

> 后面可能会迭代优化以及新增交互功能

| 配置项 | 说明 | 默认值 | 关键指数 |
| --- | --- | --- | --- |
| 网页标题自定义 | 自定义网页标题，该配置项可以改写网页的标题。支持多种匹配模式，（如**相同**，**开始于**，**结束于**，**包含**，**正则**，**通配符**）。<br />正则示例：`.*?\.vuejs\.org.*` <br />通配符示例：`*.vuejs.org*` | 无 | ★★★★★ |
...
| 标签组-复制链接模板格式 | 个人认为这是个好功能啊，可配置一键复制标签组中所有标签页的标题和链接时的模板格式。默认提供三种模板格式快捷切换，您也可根据需求自由配置模板格式（模板格式采用`Mustache`语法，一键复制时会自动替换模板中变量）。场景：比如markdown文档中添加链接的语法为 `[标题](链接)`，则可以配置模板格式为`[{{title}}]({{url}})`, 就可以将标签组中的标签页批量格式化为符合markdown语法的文本。 | `{{title}} \| {{url}}` | ★★★★ |
...
| 配置项 | 说明 | 默认值 | 关键指数 |
| --- | --- | --- | --- |
| ~~标签组和标签页操作按钮样式~~ | ~~`（已弃用）`标签组和标签页的操作按钮旧版中是文本样式，`v2.7.4`版本开始新增图标样式~~ | 图标样式 | ★★★★★ |
...
| ~~页面内容区宽度设置~~ | ~~`（已弃用）`管理后台主内容区域的宽度展示方式，默认“固定宽度”（大概1200像素，居中展示），设置为“自适应宽度”后，将以网页宽度进行自适应展示，效果上类似于居左展示~~ | 固定宽度 | ★★ |
| ~~标签页hover时是否展示tooltip~~ | ~~`（已弃用）`该配置项只是暂时保留历史交互，不推荐开启tooltip展示。~~ | 否 | ★ |

### 新标签页配置

这个模块主要是定制化浏览器原生的新标签页。

| 配置项 | 说明 | 默认值 | 关键指数 |
| --- | --- | --- | --- |
| 新标签页显示方式 | 您可以将新标签页替换为 `NiceTab 管理后台` 或者 `NiceTab 定制新标签页`，默认选项为浏览器原生的新标签页。 | 默认（恢复为浏览器默认新标签页） | ★★★★ |
...
v2.8.2 版本支持自定义远程存储目录，您可以在一个远程账号上为不同设备分配不同的目录进行数据管理。
...
### 远程容灾备份
v2.9.2 版本新增了精简版的远程容灾备份功能，在远程同步推送数据时，会先将远程数据进行备份（可自定义备份目录和备份文件名），然后再推送数据到远程。

> 备份数据会比主数据低一个版本，正常情况下，以最新的主数据为主，若主数据出现异常或丢失，则上一个版本的备份数据可能会用得上。
> 插件目前不会自动从备份数据中恢复数据，备份数据只是作为一个保障，用户可以自行登录网盘账号查看备份数据。

### 默认目录和文件名

**gists 主数据**
- 默认描述标识为：`__NiceTab_gist_key__`
- 默认列表文件名为：`__NiceTab_gist__.json`
- 默认偏好设置文件名为：`__NiceTab_settings_gist__.json`

**gists 备份数据**
- 默认描述标识为：`__NiceTab_gist_key_bak__`
- 默认列表文件名为：`__NiceTab_gist__.json`
- 默认偏好设置文件名为：`__NiceTab_settings_gist__.json`

**WebDAV 主数据**
- 默认目录名为：`__NiceTab_web_dav__`
- 默认列表文件名为：`__NiceTab_web_dav__.json`
- 默认偏好设置文件名为：`__NiceTab_settings_web_dav__.json`

**WebDAV 备份数据**
- 默认目录名为：`__NiceTab_web_dav_bak__`
- 默认列表文件名为：`__NiceTab_web_dav__.bak.json`
- 默认偏好设置文件名为：`__NiceTab_settings_web_dav__.bak.json`
