...
## 管理后台-导航栏操作
- 页面切换：可在**列表**、**偏好设置**、**快照**、**导入/导出**、**远程同步**、**回收站**这些页面快速切换。
...
点击导航栏“操作”菜单中的“创建快照”，可以一键保存**当前窗口**的完整工作环境。快照包含标签页顺序、固定状态、恢复后激活的标签页，以及原生标签组的名称、颜色和折叠状态。

管理后台新增了“快照”页面：

- 快照按日期时间自动命名，支持重命名和删除，点击“查看”按钮可查看快照中的标签组和标签页。
- 恢复快照可选择“在新窗口还原”以保留当前窗口状态；或选择“替换当前窗口”，则当前窗口将被快照内容覆盖。
- 手动快照最多保留 50 份，达到上限时可确认删除最旧快照并继续创建。
- 标签页和分组变化会实时写入崩溃恢复缓冲区；浏览器下次启动时，NiceTab 将该缓冲区保存为一份自动快照，并根据“启动浏览器时是否自动恢复”设置决定是否自动恢复。
- 某些网址因浏览器安全限制无法打开时，NiceTab 会继续恢复其他标签页，并显示成功与失败数量。

Gists/WebDAV 远程同步不包含快照数据。
