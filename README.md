...
- 类似于 `OneTab`、`N-Tab`、`KepTab` 等标签页管理插件，支持**谷歌Chrome**、**Firefox**、**微软Edge**等浏览器。
...
- 支持多种插件格式的 **导入/导出** 功能，支持导出到本地。目前支持 `NiceTab`、`OneTab`、`KepTab` 格式的交叉导入导出（比如：可选择导入OneTab格式并导出为NiceTab格式；或者将NiceTab格式导出为OneTab格式），后续可根据需求增加其他插件格式的导入导出功能。
...
### 扩展管理后台-导入导出
导入导出功能目前支持 `NiceTab` 和 `OneTab`、`KepTab` 格式的交叉导入导出。
- 可导入 `OneTab`、`KepTab` 格式的列表并解析为 `NiceTab` 格式。
- 可将 `NiceTab`格式的列表导出为 `NiceTab`、`OneTab`、`KepTab` 格式。
...
- 打开**管理后台 > 导入导出**页，可进行 NiceTab、OneTab 和 KepTab 格式的标签页导入导出操作。
