...
- 类似于 `OneTab`、`N-Tab`、`KepTab`、`Toby` 等标签页管理插件，支持**谷歌Chrome**、**Firefox**、**微软Edge**等浏览器。
...
- 支持多种插件格式的 **导入/导出** 功能，支持导出到本地。目前支持 `NiceTab`、`OneTab`、`KepTab`、`Toby` 格式的交叉导入导出（比如：可选择导入OneTab格式并导出为NiceTab格式；或者将NiceTab格式导出为OneTab格式），后续可根据需求增加其他插件格式的导入导出功能。
...
### 扩展管理后台-导入导出
导入导出功能目前支持 `NiceTab` 和 `OneTab`、`KepTab`、`Toby` 格式的交叉导入导出。
- 可导入 `OneTab`、`KepTab`、`Toby` 格式的列表并解析为 `NiceTab` 格式。
- 可将 `NiceTab`格式的列表导出为 `NiceTab`、`OneTab`、`KepTab`、`Toby` 格式。

`KepTab` 和 `Toby` 格式导入导出的截图暂未更新。
...
- 打开**管理后台 > 导入导出**页，可进行 NiceTab、OneTab、KepTab、Toby 格式的标签页导入导出操作。
...
## 更新计划
- [x] `NiceTab`、`OneTab`、`KepTab`、`Toby` 格式导入导出功能
- [x] 回收站功能
- [x] 切换皮肤功能
- [x] 支持切换亮色/暗黑主题
- [x] 支持多语言（目前暂时支持中英文切换）
...
- [x] 发送标签页操作添加快捷键。
- [x] 发送标签页支持选择指定目录。
- [x] 支持域名排除功能（支持正则匹配），排除列表中的标签页不会发送到NiceTab。 
- [x] 支持偏好设置信息的导入导出和远程同步功能。 
- [ ] 新增固定分类，集中展示所有分类中星标的标签组。
- [ ] 扩展图标右键菜单支持按需配置。
- [ ] 支持便签功能及便签远程同步。
- [ ] 其他
